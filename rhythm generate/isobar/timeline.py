...
class Channel:
	...
	def __str__(self):
		...
		self.pos, self.event["note"], self.event["dur"], self.dur_now, self.event["channel"],
		self.event["control"] if "control" in self.event else "-", self.count_now, self.count_max)
	...
	def play(self):
		...
		if "control" in values:
			...
			log.debug("control (channel %d, control %d, value %d)", values["channel"], values["control"],
			          values["value"])
