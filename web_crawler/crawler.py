...
class HymnCrawler():
	...
	def fetch_song(self, url, song_dir):
		...
		# metadata
		content_list = []
		tag_list = soup.find_all('div', {'class': 'row common-panel'})[0].find_all('div', {
			'class': 'col-xs-7 col-sm-8 no-padding'})

		for t in tag_list:
			content_list.append(t.text.strip())
		label_list = []
		tag_list = soup.find_all('div', {'class': 'row common-panel'})[0].find_all('label',
